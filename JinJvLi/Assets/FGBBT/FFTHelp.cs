...
public class FFTHelp
{
    ...
    /// <returns></returns>
    public static float[] fft(float[] array)
    {
        ...
        for (int i = 0; i < N / 2; i++)
        {
            WN[i] = new Complex((float)Math.Cos(2 * Math.PI / N * i), -1 * (float)Math.Sin(2 * Math.PI / N * i));
        }
    ...
    /// <param name="array"></param>
    /// <returns></returns>
    public static float[] dft(float[] array)
    {
        //array = FillArray(array);
        int N = array.Length;
        ...
        for (int k = 0; k < N; k++)
        {
            ...
            for (int n = 0; n < N; n++)
            {
                sum += Register[n] * (new Complex((float)Math.Cos(2 * Math.PI / N * n * k), -1 * (float)Math.Sin(2 * Math.PI / N * n * k)));
            }
            dest[k] = sum;
        }

        float[] dest2 = new float[N];
        for (int k = 0; k < N; k++)
        {
    ...
    /// <param name="array"></param>
    /// <returns></returns>
    public static float[] idft(float[] array)
    {
        //array = FillArray(array);
        int N = array.Length;
    ...
    /// <returns></returns>
    public static float[] FFTFilter(float[] array, int n)
    {
        ...
        float[] result = new float[dest2.Length];
